...
const TIERS = [
  {
    name: 'Starter', price: '₦50,000', period: '/month',
    ...
    highlight: false, badge: null, dotColor: '#2563EB',
    coverage: [
      '1 ProStack NG platform managed', 'Monthly platform health checks',
      'Uptime monitoring (99% SLA)', 'Security patches & dependency updates',
      'Monthly performance report', 'Minor bug fixes (up to 3 hrs/month)', 'Email support',
    ],
  ...
  {
    name: 'Growth', price: '₦120,000', period: '/month',
    ...
    highlight: true, badge: 'Most Popular', dotColor: '#3B82F6',
    coverage: [
      'Up to 3 ProStack NG platforms managed', 'Weekly platform health checks',
      'Advanced monitoring & alerting', 'Security patches & dependency updates',
      'Bug fixes (up to 8 hrs/month)', 'Minor feature enhancements',
      'Weekly performance reports', 'Monthly strategy call (1 hr)',
  ...
  {
    name: 'Enterprise', price: '₦200,000+', period: '/month',
    ...
    highlight: false, badge: 'Custom', dotColor: '#A78BFA',
    coverage: [
      'Unlimited platforms managed', 'Daily monitoring & health checks',
      'Dedicated account manager', 'Custom emergency response protocol',
      'Unlimited bug fixes', 'Feature development budget included',
      'Custom performance dashboards', 'Quarterly roadmap planning sessions',
      'WhatsApp + phone support', 'Staff training sessions (quarterly)',
    ],
              ...
              {TIERS.map(tier => (
                <div key={tier.name} style={{
                  ...
                  <div style={{ marginBottom: 8 }}>
                    <span className="f-display" style={{ fontSize: 40, fontWeight: 800, letterSpacing: '-.04em', color: tier.highlight ? 'var(--blue-hi)' : 'var(--text)' }}>
                      {tier.price}
                    </span>
                    <span className="f-mono" style={{ fontSize: 11, color: 'var(--muted)', letterSpacing: '.1em', marginLeft: 4 }}>{tier.period}</span>
                  </div>
                  <p className="f-mono" style={{ fontSize: 10, letterSpacing: '.1em', textTransform: 'uppercase', color: tier.highlight ? 'var(--blue-hi)' : 'var(--sub)', marginBottom: 12 }}>
                    {tier.tagline}
                  </p>
                  <p className="f-body" style={{ fontSize: 13, color: 'var(--sub)', lineHeight: 1.75, marginBottom: 28 }}>{tier.description}</p>
                  ...
                  <ul style={{ listStyle: 'none', marginBottom: 28, flex: 1 }}>
                    {tier.coverage.map(item => (
                      <li key={item} className="f-body" style={{ fontSize: 13, color: 'var(--sub)', paddingLeft: 18, position: 'relative', marginBottom: 10, lineHeight: 1.5 }}>
        ...
        <section style={{
          ...
          <div style={{ maxWidth: 1000, margin: '0 auto' }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 560 }}>
              ...
              <tbody>
                {SLA_ROWS.map((row, ri) => (
                  <tr key={row.label} style={{ borderBottom: '1px solid var(--border)', background: ri % 2 === 0 ? 'transparent' : 'rgba(255,255,255,.015)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 2 }}>
              {HOW_IT_WORKS.map((item, idx) => (
                <div key={item.step} style={{
                  ...
                  <div className="f-display text-ghost" style={{ fontSize: 56, fontWeight: 800, letterSpacing: '-.04em', lineHeight: 1, marginBottom: 20, userSelect: 'none' }}>
                    {item.step}
                  </div>
                  ...
                  {idx < HOW_IT_WORKS.length - 1 && (
                    <div style={{ position: 'absolute', top: '50%', right: -14, transform: 'translateY(-50%)', zIndex: 2, color: 'var(--blue)', fontSize: 18, fontWeight: 700 }}>→</div>
                  )}
                </div>
              ))}
            </div>
        ...
        <section style={{
          ...
          <div style={{ maxWidth: 800, margin: '0 auto' }}>
            <div style={{ marginBottom: 48 }}>
              ...
              <h2 className="f-display" style={{ fontSize: 'clamp(26px,3.5vw,40px)', fontWeight: 800, letterSpacing: '-.03em' }}>Common Questions</h2>
            </div>
